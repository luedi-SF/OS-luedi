...
all: ${BUILD}/boot/setup.o ${BUILD}/boot/boot.o ${BUILD}/kernel/kernel.bin
	...
	dd if=${BUILD}/kernel/kernel.bin of=hd.img bs=512 seek=3 count=100 conv=notrunc


${BUILD}/boot/setup.o: src/kernel/boot/asm/setup.asm
	$(shell mkdir -p ${BUILD}/boot)
	nasm $< -o $@
${BUILD}/boot/boot.o: src/kernel/boot/asm/boot.asm
	$(shell mkdir -p ${BUILD}/boot)
	nasm $< -o $@

${BUILD}/kernel/asm/head.o: src/kernel/asm/head.asm
	$(shell mkdir -p ${BUILD}/kernel/asm)
	nasm -f elf32 -g $< -o $@

${BUILD}/kernel/asm/memory.o: src/kernel/asm/memory.asm
	$(shell mkdir -p ${BUILD}/kernel/asm)
	nasm -f elf32 -g $< -o $@
...
${BUILD}/kernel/kernelDBG.bin: ${BUILD}/kernel/asm/head.o ${BUILD}/kernel/asm/memory.o ${BUILD}/kernel/main.o
